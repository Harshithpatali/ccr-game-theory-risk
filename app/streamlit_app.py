...
from src.explainer import (  # noqa: E402
    explain_page,
...
st.markdown(
    ...
    <div class="ccr-hero">
      ...
      <p>Counterparty exposure decision-support · Bayesian game theory · Monte Carlo &amp; stress analytics</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
try:
    cp_df = pd.DataFrame(fetch("/counterparties"))
except Exception as exc:  # noqa: BLE001
    st.error(f"Failed to load counterparty data. _Details: `{exc}`_")
    st.stop()
...
k1.metric("Total EAD", gbp(metrics.get("total_ead", 0), 2))
k2.metric("Counterparties", f"{metrics.get('counterparties', len(cp_df)):,}")
k3.metric("Anomalies", f"{metrics.get('anomalies', 0):,}")
k4.metric("Trades", f"{metrics.get('trades', 0):,}")
k5.metric("Mean utilisation", pct(metrics.get("mean_utilisation", 0)))


def stakeholder_box(page_key: str, context: dict | None = None, expanded: bool = False):
    """Clear explanation panel at the top of each tab."""
    with st.expander("ℹ️ What does this page mean? (stakeholder guide)", expanded=expanded):
        st.markdown(explain_page(page_key, context))
        if os.getenv("GROQ_API_KEY", "").strip():
            st.caption("Explanation generated / refined with Groq · model openai/gpt-oss-120b")
        else:
            st.caption(
                "Static guide. Set GROQ_API_KEY in the environment (or .env) to enable live "
                "plain-language briefings from Groq openai/gpt-oss-120b."
            )


stakeholder_box("overview", metrics, expanded=True)
with st.expander("📖 Metric glossary"):
    st.markdown(glossary_markdown())

# ============================================================================
#  Sidebar — risk-appetite filters (need the counterparty table)
# ============================================================================
with st.sidebar:
    st.divider()
...
def render_risk_monitor(df: pd.DataFrame):
    ...
    st.caption("Click any column header to sort · use the column menus to filter and search.")
...
def render_stress():
    try:
        ...
        st.error(f"Failed to load stress results. _Details: `{exc}`_")
        return
    ...
    base_ead = float(metrics.get("total_ead", 0) or 0)
...
def render_transitions():
    try:
        tm = fetch("/transitions")
    except Exception as exc:  # noqa: BLE001
        st.error(f"Failed to load transitions. _Details: `{exc}`_")
        return
...
def render_drilldown(df: pd.DataFrame):
    ...
    st.caption("Use as an input to credit discussion, not as an automatic order.")


with tabs[5]:
    stakeholder_box("drilldown")
